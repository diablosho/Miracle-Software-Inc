...
public class JonesTreasurePath extends SimFrame
{
	...
	{
		JonesTreasurePath app = new JonesTreasurePath("SimplePath", terrainFile);	//	Create the app
	...
	public JonesTreasurePath(String frameTitle, String imageFile)			//	Create the Mainframe
	{
		...
		usageItem.addActionListener(new ActionListener()
		{
			...
			{
				JOptionPane.showMessageDialog(JonesTreasurePath.this, "An informational message string \n"
		...
		authorItem.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				JOptionPane.showMessageDialog(JonesTreasurePath.this, "Brian Jones \n" + "brian.jones.476@my.csun.edu \n"
	...
	public void DrawWaypoints()	//	Draw Waypoints to screen in grid pattern, and draw edges denoting potential neighboring-paths
	{
		...
		while (it.hasNext())
		{
			...
			else if (isGoldWaypoint(waypoint))	animatePanel.addPermanentDrawable(markerGold);
	...
	public void GetStartingWaypoint()	//	Retrieve starting Waypoint from user
	{
		...
		do
		{
			...
			startingWaypoint = data.table.get((new Waypoint(point)).getHash());
	...
	public void GetEndingWaypoint()	//	Retrieve ending Waypoint from user
	{
		...
		do
		{
			...
			endingWaypoint = data.table.get((new Waypoint(point)).getHash());
	...
	public void setSimModel()		//	Initialize the Simulation Model variables, and then create the Player bot
	{
		...
		data.SetStartAndFinishWaypoints(startingWaypoint, endingWaypoint);
	...
	public boolean EvaluateNeighbors()		//	Determine which neighbor of currentWaypoint is optimal
	{
		for (int neighborIndex = 0; neighborIndex < player.currentWaypoint.countNeighbors; neighborIndex++)
		{
			...
			Waypoint waypointNeighbor = data.table.get(neighborKey);
		
			if (!CLOSED.containsKey(neighborKey))
			{
				//	For Each Waypoint Neighbor, Add the Heuristic to their Cost
				waypointNeighbor.costSoFar = player.currentWaypoint.costSoFar + player.currentWaypoint.distance(waypointNeighbor.Location);
	...
	public void WalkTheDots()
	{
		...
		while (!shortestPath.isEmpty())
		{
			...
			if (isMapWaypoint(player.currentWaypoint) && !player.tmapRunning) // Currently on a Treasure-Map Waypoint
			{
				...
				animatePanel.clearTemporaryDrawables();
				player.endingWaypoint = data.table.get(new Waypoint(new Point(	player.currentWaypoint.mapX,
																				player.currentWaypoint.mapY))
																				.getHash());
				data.SetStartAndFinishWaypoints(player.currentWaypoint, player.endingWaypoint);
	...
	public void RemoveTreasureMap()
	{
		...
		data.table.get(player.currentWaypoint.getHash()).mapX = 0;
		data.table.get(player.currentWaypoint.getHash()).mapY = 0;
	...
	public boolean isGoldWaypoint(Waypoint waypoint)
	{
